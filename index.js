...
const defaultSettings = {
    ...
    tags: {},
    customOrder: {},
    folderColors: {}, // Уникальные цвета папок
    folderSort: {}    // Режим сортировки папок
};
...
function onColorChange() {
    ...
    saveSettingsDebounced();
    applyColors();
    buildFolderUI(); // Перерисовываем, чтобы применить прозрачность к кастомным папкам
}
...
function getChatsGroupedByCharacter() {
    ...
    Object.keys(grouped).forEach(charName => {
        ...
        grouped[charName].sort((a, b) => {
            ...
            if (pinA !== pinB) return pinB - pinA; // Пины всегда сверху
            ...
            if (sortMode === 'alpha') {
                ...
                return a.originalIndex - b.originalIndex; // Исходный порядок Таверны = по дате
            } else {
                // Пользовательский порядок (Custom)
    ...
    const tryBuild = () => {
        ...
        Object.entries(grouped).forEach(([charName, chats]) => {
            ...
            chats.forEach(chat => {
                ...
                // Извлечение счетчика сообщений из атрибута title
                let msgCountStr = "";
                ...
                $editTray.append($renameArea, $tagArea, $noteArea);
                ...
                $wrapper.on("dragstart", function(e) {
                    if (sortMode !== 'custom') return e.preventDefault(); // Запрет Drag если не кастомная сортировка
